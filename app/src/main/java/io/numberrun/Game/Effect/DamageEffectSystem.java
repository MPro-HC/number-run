...
public class DamageEffectSystem implements GameSystem {
    ...
    public void update(World world, float deltaTime) {
        ...
        for (Entity entity : damageEffects) {
            ...
            System.out.println("DamageEffectSystem: Easing progress: " + easing.easeOutCubic() + ", opacity: " + easeValue);
            rectangle.setOpacity(easeValue);

            // Easing が完了したらエンティティを削除する
            if (easing.isFinished()) {
                System.out.println("DamageEffectSystem: Removing completed damage effect entity.");
                entity.destroy();
            }
        }
    }
